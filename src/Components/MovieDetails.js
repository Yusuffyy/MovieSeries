...
const MovieDetails = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await axios.get(`https://api.themoviedb.org/3/movie/${id}?language=tr-TR&api_key=${API_KEY}`);
                setItem(res.data);
            } catch (error) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await axios.get(`https://api.themoviedb.org/3/movie/${id}/videos?language=en-US&api_key=${API_KEY}`);
                setItem3(res.data);
            } catch (error) {
                console.error('Error fetching data:', error);
            }
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                const res = await axios.get(`https://api.themoviedb.org/3/movie/${id}/credits?language=tr-TR&api_key=${API_KEY}`);
                setItem2(res.data);
            } catch (error) {
                console.error('Error fetching data:', error);
            }
    ...
    return (
        ...
        >
                <div
              className="inset-0 bg-black opacity-60" 
             style={{
            ...
            ></div>
                ...
                <div className="flex gap-2 text-white brightness-200 justify-center py-12" >   
                    ...
                    <div className='flex p-2 w-3/5 mx-14' >
                        <div className='text-white'>
                            ...
                            <div className='grid grid-cols-2 items-center py-10 gap-4'>
                                    ...
                                    .filter(video => video.type === "Trailer" && video.official === true ) // Sadece fragman türündeki videoları filtrele
                    ...
                    <div className='flex flex-cols 1 justify-center w-52 mt-2 h-64'>
                        <div className="flex h-52">
                            <div className=''>
                                ...
                                <div className='py-2'>
                                    <h1 className=' text-white font-bold'>Bütçe</h1>
                                    <p>${item.budget}</p>
                                </div>
                                <div className='py-2'>
                                    <h1 className=' text-white font-bold'>Hasılat</h1>
                                    <p>${item.revenue}</p>
                                </div>
                ...
                <div className=" max-w-screen-2xl flex flex-row auto-rows-max gap-6 mx-auto brightness-75 rounded-lg p-4">
