...
const TopRatedSeries = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await axios.get(
          `https://api.themoviedb.org/3/tv/top_rated?language=tr-TR&page=${page}&api_key=${API_KEY}`
        );
  ...
  return (
   <div>
     <div className='max-w-screen-2xl mx-auto p-4 mt-10'>
      <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-5 border-2 border-white gap-7'>
        {item.map((x) => (
          <div key={x.id} className='hover:transform transition duration-300 hover:scale-95 cursor-pointer rounded-lg shadow-2xl items-center ' onClick={() => navigate(`/${x.id}/series-details`)}>
           ...
           <div className={`absolute flex items-center justify-center w-14 h-14 border-2 p-3 text-2xl text-extrabold brightness-200 rounded-full ${
            ...
            <img src={`${poster}${x.poster_path}`} className='rounded hover:text-red-500' alt={x.name} />
            <div className='flex justify-center items-center py-4'>
            <h2 className='w-64 font-semibold text-black hover:text-red-500 text-center'>{x.name}</h2>
            </div>
          </div>
        ))}
      </div>
