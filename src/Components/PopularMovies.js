    ...
    const fetchData = async () => {
      try {
        ...
        const res = await axios.get(
          `https://api.themoviedb.org/3/movie/popular?language=tr-TR&page=${page}&api_key=${API_KEY}`
        );
   ...
   <div>
     <div className='max-w-screen-2xl mx-auto p-4 mt-10'>
      <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-5 gap-7 '>
        {item.map((x) => (
          <div key={x.id} className='hover:transform transition duration-300 hover:scale-95 cursor-pointer rounded-lg shadow-2xl items-center' onClick={() => navigate(`/${x.id}/movie-details`)}>
